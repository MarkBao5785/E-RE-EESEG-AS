...
if __name__ == '__main__':
    ...
    for i in range(runing_times):  
        ...
        for t in range(2000):
            ...
            if t % 5 == 0:
                print("round:"+str(t)+" seg:regret:"+str(sum_regret_seg)+" average_regret:"+str(sum_regret_seg/(t+1))+" loss1:"+str(loss_1_seg)+" loss2:"+str(loss_2_seg)+" loss3:"+str(loss_3_seg))
                printtime()
                # wandb.log({"seg_sum":df})
    regrets_all.append(regrets_seg)
    path = os.getcwd()
    np.save('{}/results/seg_movie.npy'.format(path), regrets_all)
